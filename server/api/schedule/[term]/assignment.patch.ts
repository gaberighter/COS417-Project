...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<Partial<IAssignment> & { courseId: string }>(event);
  if (!body.courseId) {
    throw createError({ statusCode: 400, statusMessage: "courseId is required to identify assignment" });
  }
  ...
  if (assignmentIndex < 0) {
    throw createError({ statusCode: 404, statusMessage: `Assignment not found: ${body.courseId}` });
  }
