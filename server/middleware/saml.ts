...
function getProviders() {
  ...
  const privateKeyPath = process.env.SAML_PRIVATE_KEY_PATH || 'wildcard_covenant_edu.key'
  const certificatePath = process.env.SAML_PUBLIC_CERT_PATH || 'wildcard_covenant_edu.pem'
...
export default defineEventHandler(async (event: H3Event) => {
  ...
  if (samlIndex !== -1) {
    ...
    switch (samlAction) {
        ...
        return new Promise((resolve, reject) => {
          sp.create_login_request_url(
            ...
            (err: Error | null, login_url: string) => {
              if (err) {
                return reject(createError({ statusCode: 500, statusMessage: err.message }))
              }
      ...
      case 'assert': {
        ...
        const requestPayload = method === 'POST' ? await readBody(event) : getQuery(event)
        ...
        return new Promise((resolve, reject) => {
          const onAssert = async (err: Error | null, samlResponse: any) => {
            try {
              ...
              const { name_id, session_index, attributes: samlUser } = samlResponse.user
              ...
              for (const field in samlUser) {
                if (Array.isArray(samlUser[field]) && samlUser[field].length === 1) {
              ...
              return resolve(sendRedirect(event, process.env.SAML_REDIRECT_TO || '/'))
            } catch (e: any) {
              return reject(createError({
                statusCode: 500,
                statusMessage: e?.message || 'Unexpected error during SAML assertion',
              }))
            }
          }

          if (method === 'POST') {
            return sp.post_assert(idp, { request_body: requestPayload }, onAssert)
          }
          return sp.redirect_assert(idp, { request_body: requestPayload }, onAssert)
        })
      }
  ...
  if (apiIndex !== -1 && !urlObj.pathname.endsWith('/api/_auth/session')) {
    ...
    const isRegistrarOnly = REGISTRAR_ONLY.some(route => urlObj.pathname.startsWith(route))
    const isFacultyRoute = FACULTY_OR_REGISTRAR.some(route => urlObj.pathname.startsWith(route))
